...
inline void AddFuncToEnv(const EnvPtr &env, const char *id,
        const IdList &args, Obj obj, Func func, Args ...other) {
...
void Interpreter::InitEnvironment() {
    ...
    auto add_op = [this, &env](const char *id, Operator op, bool unary) {
        if (unary) {
            AddFuncToEnv(env, id, {"l"},
                this, &Interpreter::IonCalcOp, op);
        }
        else {
            AddFuncToEnv(env, id, {"l", "r"},
                this, &Interpreter::IonCalcOp, op);
        }
    };
...
ValPtr Interpreter::IonCalcOp(const EnvPtr &env, Operator op) {
    ...
    auto new_val = [&env](int v) {
        return std::make_shared<Value>(env, v);
    };
    // do calculation
    switch (op) {
